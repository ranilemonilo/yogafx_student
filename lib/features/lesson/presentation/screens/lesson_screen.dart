...
class _LessonScreenState extends ConsumerState<LessonScreen> {
  ...
  Future<void> _replaceLessonRoute(
    ...
    if (startFromBeginning) params.add('fromStart=1');
    ...
    return PopScope(
      ...
      child: Scaffold(
        ...
        body: lessonAsync.when(
          loading: () => widget.autoOpenFullscreen
              ? const _FullscreenTransitionLoader()
...
class _LessonContentState extends ConsumerState<_LessonContent>
  ...
  bool _isOpeningAutoFullscreen = false;
  ...
  void initState() {
    ...
    if (_hasPlayableVideo && _isVideoUnlocked) {
      _initVideo();
    } else if (widget.autoOpenFullscreen) {
  ...
  Future<void> _initVideo() async {
    ...
    try {
      ...
      if (widget.autoOpenFullscreen &&
        ...
        _didAutoOpenFullscreen = true;
        setState(() => _isOpeningAutoFullscreen = true);
  ...
  @override
  Widget build(BuildContext context) {
    final shouldHideInlinePageForFullscreen = widget.autoOpenFullscreen &&
        (!_didAutoOpenFullscreen || _isOpeningAutoFullscreen);

    if (shouldHideInlinePageForFullscreen) {
      return const _FullscreenTransitionLoader();
    }
    ...
    final compactBody = RefreshIndicator(
  ...
  Widget build(BuildContext context) {
    return ValueListenableBuilder<VideoPlayerValue>(
      ...
      builder: (context, value, _) {
        ...
        final controlsBar = _VideoControlsBar(
          duration: duration,
          position: position,
          isPlaying: value.isPlaying,
          isMuted: isMuted,
          onTogglePlayback: () => _runControlAction(widget.onTogglePlayback),
          onSeek: (position) => _runControlAction(
            () => widget.onSeek(position),
          ),
          onToggleMute: () => _runControlAction(widget.onToggleMute),
          onOpenFullscreen: () => _runControlAction(widget.onOpenFullscreen),
        );
        final thinProgressBar = _VideoThinProgressBar(
          duration: duration,
          position: position,
          visible: !_controlsVisible,
          onSeek: (position) => widget.onSeek(position),
        );

        final controlsOverlay = IgnorePointer(
        ...
        final videoSurface = GestureDetector(
...
class _VideoThinProgressBar extends StatelessWidget {
  final Duration duration;
  final Duration position;
  final bool visible;
  final Future<void> Function(Duration position) onSeek;

  const _VideoThinProgressBar({
    required this.duration,
    required this.position,
  ...
  Future<void> _seekFromDx(double dx, double width) async {
    if (width <= 0) return;
    final ratio = (dx / width).clamp(0.0, 1.0);
    final targetMillis = (duration.inMilliseconds * ratio).round();
    await onSeek(Duration(milliseconds: targetMillis));
  }
          ...
          child: LayoutBuilder(
            builder: (context, constraints) {
              return GestureDetector(
                ...
                onHorizontalDragStart: (details) => _seekFromDx(
                  details.localPosition.dx,
                  constraints.maxWidth,
                ),
            ...
            Positioned.fill(
              child: AbsorbPointer(
                child: ColoredBox(
                  ...
                  child: const _FullscreenTransitionLoader(),
                ),
              ),
            ),
        ],
...
class _FullscreenTransitionLoader extends StatelessWidget {
  const _FullscreenTransitionLoader();
